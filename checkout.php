	...
	<div class="container-fluid">
		<div class="row-checkout">
			...
			if(isset($_SESSION["uid"])){
				...
				<label for="fname">Chấp nhận các loại thẻ sau:</label>
				...
				<label><input type="CHECKBOX" name="q" class="roomselect" value="conform" required>Thanh toán khi nhận hàng.
				</label>';
				$i=1;
				$total=0;
			...
			<div class="col-25">
				<div class="container-checkout">
					...
					if (isset($_POST["cmd"])) {
						...
						<table class='table table-condensed' style='background-color:#F3F781;border:2px solid black;border-radius:8px'>
						<thead><tr>
						<th>STT</th>
						<th>  Tên SP</th>
						<th>	SL	</th>
						...
						while($i<=$total_count){
							...
							$quantity_ = $_POST['quantity_'.$i];							
							$amount_ = $_POST['amount_' .$i];
						// <!-- Lấy Gía cho 1 sản phẩm-->
							$price_ = $amount_ * $quantity_;
						// tính giá của 1 sản phẩm theo số lượng của n
							$total=$total+$price_ ;
						// tính giias tất cả sp trong giỏ
							$sql = "SELECT product_id FROM products WHERE product_title='$item_name_'";
							$query = mysqli_query($con,$sql);
							$row=mysqli_fetch_array($query);
							$product_id=$row["product_id"];
							...
							<tr><td><p>$item_number_</p></td><td><p style='font-weight:bolder'>$item_name_</p></td><td ><p>$quantity_</p></td> <td ><p>$amount_</p></td> <td ><p>$price_</p></td></tr>";
						...
						<h4>Tổng cộng:<span class='price' style='color:black'><b style='color:red'>$total VND</b></span></h4>";
		...
		<div class="row">
			<div class="col-md-12">
				<div class="newsletter">
					<p>Đăng nhập để gửi phản hồi đến chúng tôi.</p>
					<form >
						<input class="input" type="email" placeholder="Nhập Email...">
						<button class="newsletter-btn"><i class="fa fa-envelope"></i> Gửi</button>
					</form>
