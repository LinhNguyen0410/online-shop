...
if(isset($_POST["addToCart"])){
	...
	if(isset($_SESSION["uid"])){
		...
		if($count > 0){
			...
			<b>Sản phẩm đã được thêm. Vào <a href='cart.php'><i class='fa fa-shopping-cart'></i><b> giỏ hàng</b></a> để tiếp tục mua sắm..!</b>
			</div>";
		} else {
			...
			if(mysqli_query($con,$sql)){
				...
				<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
				<b>Thêm sản phẩm vào giỏ thành công. Vào <a href='cart.php'><i class='fa fa-shopping-cart'></i><b> giỏ hàng</b></a> để tiếp tục mua sắm..!</b>
				</div>";
			}
		}
	}else{
	...
	if (isset($_POST["getCartItem"])) {
		...
		if (mysqli_num_rows($query) > 0) {
			...
			while ($row=mysqli_fetch_array($query)) {
				...
				<h4 class="product-price"><span class="qty">'.$n.'</span>'.$product_price .' VND</h4>
				</div>
			...
			<small class="qty">'.$n.' sản phẩm trong giỏ</small>
			<h5>'.$total_price.' VND</h5>
			</div>'
			?>
	...
	if (isset($_POST["checkOutDetails"])) {
		if (mysqli_num_rows($query) > 0) {
			...
			while ($row=mysqli_fetch_array($query)) {
				...
				<div class="col-sm-4 "><img src="product_images/'.$product_image.'" style="height: 100px;width:120px;"/>
			...
			if (!isset($_SESSION["uid"])) {
				...
				<a href="" data-toggle="modal" data-target="#Modal_register" class="btn btn-success"> Đặt Hàng</a></td>
				</tr>
				</tfoot>
				...
				<input type="submit" id="submit" name="login_user_with_product" name="submit" class="btn btn-success" value="Đặt hàng">
				</form></td>
...
if (isset($_POST["updateCartItem"])) {
	...
	if(mysqli_query($con,$sql)){
		...
		<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
		<b>Đã cập nhật thông tin sản phẩm</b>
		</div>";
		exit();
	}
}
